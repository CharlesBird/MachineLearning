...
# 1.数据集
N_SAMPLES = 2000  # 采样点数
TEST_SIZE = 0.2  # 测试数量比率
# 利用工具函数直接生成数据集
X, y = datasets.make_moons(n_samples=N_SAMPLES, noise=0.2, random_state=666)
print(X.shape, y.shape)
# 将2000 个点按着8:2 分割为训练集和测试集
...
def make_plot(X, y, plot_name, file_name=None, XX=None, YY=None, preds=None, dark=False):
    ...
    # 绘制散点图，根据标签区分颜色
    plt.scatter(X[:, 0], X[:, 1], c=y.ravel(), s=40, cmap=cm.Spectral, edgecolors='none')
    plt.savefig(file_name)
    plt.close()

# 调用make_plot 函数绘制数据的分布，其中X 为2D 坐标，y 为标签
make_plot(X, y, "Classification Dataset Visualization ", file_name='dataset.svg')
plt.show()


# 2.网络层
"""
通过新建类Layer 实现一个网络层，需要传入网络层的数据节点数，输出节点数，激活函数类型等参数，
权值weights 和偏置张量bias 在初始化时根据输入、输出节点数自动生成并初始化
"""
class Layer:
    # 全连接网络层
    def __init__(self, n_input, n_neurons, activation=None, weights=None, bias=None):
        """
        :param n_input: 输入节点数
        :param n_neurous: 输出节点数
        :param activation: 激活函数类型
        :param weights: 权值张量，默认类内部生成
        :param bias: 偏置，默认类内部生成
        """
        # 通过正态分布初始化网络权值，初始化非常重要，不合适的初始化将导致网络不收敛
        self.weights = weights if weights is not None else np.random.randn(n_input, n_neurons) * np.square(1 / n_neurons)
...
# 3.网络模型
"""
实现单层网络类后，我们实现网络模型的类NeuralNetwork，它内部维护各层的网络层
Layer 类对象，可以通过add_layer 函数追加网络层，实现如下
"""
class NeuralNetwork:
    # 神经网络大类
    def __init__(self):
...
nn.add_layer(Layer(25, 2))  # 输出层, 25=>2
